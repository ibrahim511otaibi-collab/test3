...
async function submitEvaluation() {
    ...
    try {
        const replyObj = {
            ...
            date: new Date().toISOString(),
            status: 'مغلق ومُقيم',
            reply: `✅ تم إغلاق وتقييم التكليف من الإدارة\nتقييم الإدارة: ${managerScore}/70\nسرعة الإنجاز: ${currentSpeedScore}/30\nالنسبة المئوية: ${total}%`,
            attachmentUrl: ''
        };
...
async function managerActionEval(actionType) {
    ...
    if (actionType === 'مغلق ومُقيم') {
        const total = parseInt(score) + speedScore;
        let qualityWord = score == '70' ? 'ممتاز' : (score == '50' ? 'جيد جداً' : 'جيد');
        prefix = `✅ تم التقييم والإغلاق بنسبة (${total}%)\nسرعة الإنجاز: ${speedScore}/30 | جودة المخرجات: ${qualityWord} (${score}/70)\nملاحظة الإدارة: `;
    }
